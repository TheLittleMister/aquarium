...
function load_profile_data(userID) {
	...
	$.ajax({
		...
		success: function (response) {
			...
			try { document.querySelector("#userNote").innerHTML = `<i class="fas fa-sticky-note fa-fw w3-margin-right w3-large w3-text-blue" style="color: steelblue !important;"></i>${response["user"][0]["note"]}` } catch (error) { console.log("Es otra noche más, de caminar.") };
		},
	});
}
